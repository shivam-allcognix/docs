...
description: 'A complete platform for running AI models through API and web interface.'
icon: 'globe'
---
 

## What is Model Market?
Model Market is an all-in-one AI model execution platform that lets developers generate text, images, audio, video, and code using simplified API.
...
<Steps>
  ...
  <Step title="Unified Request Format">
    You send a generation or streaming request to the endpoint ```/v1/mmk-generate``` or ```/v1/mmk-chat```based on the model type.
  </Step>
...
Model Market offers multiple API endpoint:
...
Similar format, every generative model, unified response.


```
POST /v1/mmk-chat
```
This endpoint intelligently handles streaming or chat models.
You need to specify:
- [x] type — "text" | "image" | "video" | "audio" | "code"
- [x] model — provider model name
- [x] input — prompt or instructions
...
Similar format, every streaming model, unified response.
...
Access multiple models, choose the best and use the endpoint and one request structure for your use case.
...
graph TB
    ...
    E --> I[Result Formatter/LiteLLM processing/Cost calculation/Guardrails]
    F --> I
    G --> I
    H --> I
